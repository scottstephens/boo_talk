...
class RepeatItMacro(AbstractAstMacro):
	...
	override def Expand(input as MacroStatement) as Statement:
		if input.Arguments.Count != 1:
			self.Context.Errors.Add(CompilerError(input.LexicalInfo,"RepeatIt macro takes exactly one argument."))
		rep_expr = input.Arguments[0]
		if rep_expr.NodeType != NodeType.IntegerLiteralExpression:
			self.Context.Errors.Add(CompilerError(input.LexicalInfo,"RepeatIt macro takes one integer literal expression as an argument."))
			return null
